...
echo "Downloading docker-compose.yml (beta channel)..."
curl -fsSL "$RELEASES_REPO/docker-compose.beta.yml" -o docker-compose.yml
...
if [ ! -f .env ]; then
  ...
  cat > .env << ENVEOF
# PortCraft BETA configuration — generated by installer
...
echo "Starting PortCraft Beta..."
docker compose up -d

# ── Done ─────────────────────────────────────────────────────────────

echo ""
echo "  PortCraft BETA is running!"
if [ "$HTTP_PORT" = "80" ]; then
...
echo ""
echo "  This is the BETA channel — features may be unstable."
echo "  Do not use beta for production workloads."
echo ""
